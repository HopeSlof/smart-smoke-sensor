...
(function (global) {
  ...
  const Settings = {
    ...
    _bind() {
      ...
      // ========== Tab 9: 摄像头管理 ==========
      on('s-cam-search', 'input', debounce(() => this._loadCameraList(), 300));
      on('s-cam-add', 'click', () => this._openCameraForm(null));
      $('s-cam-tbody')?.addEventListener('click', (e) => this._delegateCameraRowAction(e));
    },
    ...
    _switchTab(tabId) {
      ...
      if (tabId === 'tab-camera') this._loadCameraList();
    },
    ...
    async _submitChangePassword() {
      ...
      try {
        if (global.DashboardApi?.Settings) {
          ...
          global.UI?.Toast?.error('密码接口未就绪，请检查系统配置。');
        }
      } catch (err) {
    ...
    async _loadCommunityList() {
      ...
      this._communityLoadError = false;
      ...
      try {
        ...
        try {
          ...
          console.warn('[settings] Community.getList 失败：', err);
          // 不用前端演示常量冒充真实社区；明确展示加载失败，避免误操作。
          page = { total: 0, records: [] };
          this._communityLoadError = true;
        }
        page = page || {};
        ...
        if (!this._community.records.length) {
          const emptyText = this._communityLoadError
            ? '社区数据加载失败，请检查后端连接后重试'
            : `暂无社区数据${keyword ? '（当前搜索条件：' + keyword + '）' : ''}`;
          tbody.innerHTML = `<tr><td colspan="5" class="adm-empty">${emptyText}</td></tr>`;
        } else {
    ...
    async _auditUser(u, approve) {
      try {
        // 小区管理员审核通过且未指定小区时：先弹窗选择管理小区
        if (approve && u.role === 'COMMUNITY_ADMIN' && !u.communityId) {
          const communityId = await this._pickCommunityForAdmin(u);
          if (!communityId) {
            global.UI?.Toast?.info('已取消审核，需先指定管理小区');
            return;
          }
          // 先设置管理小区，再审核通过
          await global.DashboardApi.AdminUser.update(u.id, { communityId: Number(communityId) });
          await global.DashboardApi.AdminUser.audit(u.id, true);
          global.UI?.Toast?.success('已通过审核并指定管理小区');
        } else {
    ...
    /**
     * 弹窗选择管理小区（审核小区管理员时调用）
     * @returns {Promise<string|null>} 选中的 communityId，取消返回 null
     */
    async _pickCommunityForAdmin(u) {
      return new Promise(async (resolve) => {
        const id = 'ui-pick-community-modal';
        ...
        el.style.cssText = 'align-items:center;justify-content:center;display:flex;';
        el.innerHTML = `
          <span class="m-corner-tr"></span><span class="m-corner-bl"></span>
          <div style="width:420px;max-width:92vw;">
            <div class="modal-header">
              <div class="modal-title-wrap">
                <span class="modal-title-bar" style="background:var(--cyan);box-shadow:0 0 6px rgba(0,229,255,0.6);"></span>
                <div>
                  <div class="modal-title">指定管理小区</div>
                </div>
              </div>
            </div>
            <div class="modal-body" style="padding:18px 22px;">
              <div style="font-size:13px;line-height:1.8;color:var(--text-muted);margin-bottom:12px;">
                用户「<b style="color:var(--text-main);">${(u.username || '').replace(/[<>&]/g, c => ({'<':'&lt;','>':'&gt;','&':'&amp;'}[c]))}</b>」申请成为小区管理员，请指定其负责管理的小区：
              </div>
              <select id="pick-community-select" class="form-input" style="width:100%;padding:8px 12px;border-radius:6px;border:1px solid var(--border-cyan);background:rgba(0,0,0,0.3);color:var(--text-main);font-size:13px;">
                <option value="">— 请选择小区 —</option>
              </select>
            </div>
            <div class="modal-footer" style="padding:14px 22px 18px;">
              <button type="button" class="btn btn-ghost" data-act="cancel">取消</button>
              <button type="button" class="btn btn-primary" data-act="ok">确认并审核通过</button>
            </div>
          </div>`;
        ...
        // 加载小区列表到下拉框
        const sel = el.querySelector('#pick-community-select');
        try {
          let list = [];
          try {
          ...
          if (!list.length) {
            sel.innerHTML = '<option value="">小区列表加载失败，请刷新重试</option>';
          }
        } catch (e) {
          sel.innerHTML = '<option value="">小区列表加载失败，请刷新重试</option>';
        }
        ...
        el.querySelector('[data-act="ok"]').addEventListener('click', () => {
          const val = sel.value;
          if (!val) { global.UI?.Toast?.warning('请先选择一个小区'); return; }
          done(val);
        });
      ...
      this._miniDialog({
        ...
        bodyHtml: `
          <div class="form-grid" style="grid-template-columns:1fr 1fr">
            ...
            <div class="form-field"><div class="form-label">归属小区 <small>* 居民必填；小区管理员可审核后指定；消防员可留空</small></div>
        ...
        onSubmit: async () => {
          ...
          if (role === 'RESIDENT' && !communityId) throw new Error('居民必须选择归属小区');
          if (!isEdit) {
    ...
    async _loadCameraList() {
      const tbody = $('s-cam-tbody');
      if (!tbody) return;
      const keyword = ($('s-cam-search') ? $('s-cam-search').value : '').trim().toLowerCase();
      const isSys = this._isSysAdmin();
      const session = this._getSession();
      const canEdit = isSys || (session && session.role === 'community_admin');

      // 非管理员隐藏创建按钮
      const addBtn = $('s-cam-add');
      ...
      try {
        ...
        try {
          ...
          console.warn('[settings] Camera.list 失败：', err);
          tbody.innerHTML = '<tr><td colspan="6" class="adm-empty">摄像头数据加载失败，后端接口可能未部署</td></tr>';
          return;
        }
        ...
        if (!cameras.length) {
          tbody.innerHTML = '<tr><td colspan="6" class="adm-empty">暂无摄像头数据</td></tr>';
          return;
        }
        ...
        let communityMap = {};
        try {
          const commPage = await global.DashboardApi.Community.getList({ pageSize: 999 });
          if (commPage && commPage.records) {
            commPage.records.forEach(r => { communityMap[r.id] = r.name; });
          }
        } catch (_) {}

        tbody.innerHTML = cameras.map(c => {
          const commName = communityMap[c.communityId] || ('#' + c.communityId);
          const deviceText = c.deviceId
            ? '<b>设备#' + c.deviceId + '</b>' + (c.deviceName ? ' ' + c.deviceName : '')
            : '<span style="color:var(--text-dim);">未绑定</span>';
          const statusText = c.status === 'online' ? '<span style="color:var(--green);">在线</span>'
            : c.status === 'offline' ? '<span style="color:var(--red);">离线</span>'
            : '<span style="color:var(--text-dim);">未知</span>';
          return `
            <tr data-id="${c.id}">
              <td><b>${c.name || '-'}</b></td>
              <td>${c.location || '-'}</td>
              ...
              <td>
                <div class="row-actions">
                  <button class="link-btn" data-act="cam-edit" ${canEdit ? '' : 'disabled'}>编辑</button>
                  <button class="link-btn" data-act="cam-bind" ${canEdit ? '' : 'disabled'}>${c.deviceId ? '更换绑定' : '绑定设备'}</button>
                  ${c.deviceId ? `<button class="link-btn" data-act="cam-unbind" ${canEdit ? '' : 'disabled'}>解绑</button>` : ''}
                  <button class="link-btn danger" data-act="cam-del" ${canEdit ? '' : 'disabled'}>删除</button>
                </div>
              </td>
            </tr>`;
        }).join('');
      } catch (err) {
        console.error('[settings] 摄像头列表加载失败：', err);
        tbody.innerHTML = '<tr><td colspan="6" class="adm-empty">加载失败：' + (err && err.message || '未知错误') + '</td></tr>';
      }
    },

    _delegateCameraRowAction(e) {
      ...
      if (act === 'cam-edit') {
        // 找到摄像头数据
        const camera = this._findCameraFromRow(tr);
        if (camera) this._openCameraForm(camera);
      } else if (act === 'cam-del') {
        const camera = this._findCameraFromRow(tr);
        if (camera) this._deleteCamera(camera);
      } else if (act === 'cam-bind') {
        const camera = this._findCameraFromRow(tr);
        if (camera) this._openCameraBindForm(camera);
      } else if (act === 'cam-unbind') {
        const camera = this._findCameraFromRow(tr);
        if (camera) this._unbindCameraDevice(camera);
      }
    },

    _findCameraFromRow(tr) {
      if (!tr) return null;
      const id = tr.getAttribute('data-id');
      const cells = tr.querySelectorAll('td');
      // 从行内容重建 camera 对象（简单字段）
      return {
        id,
        name: cells[0] ? cells[0].textContent.trim() : '',
        location: cells[1] ? cells[1].textContent.trim() : '',
      };
    },

    _openCameraForm(camera) {
      const isEdit = !!camera;
      const title = isEdit ? '编辑摄像头' : '创建摄像头';
      const session = this._getSession();
      const isSys = this._isSysAdmin();

      // 创建 Modal
      const modalId = 'camera-form-modal';
      let modal = $(modalId);
      ...
      modal = create('div', {
        ...
        create('div', { class: 'modal-header' }, [
          create('div', { class: 'modal-title-wrap' }, [
            ...
            create('div', {}, [
              create('div', { class: 'modal-title' }, title),
              create('div', { class: 'modal-sub' }, '填写摄像头信息，可绑定到传感器设备'),
            ]),
          ]),
          create('button', {
            class: 'modal-close',
            onclick: () => global.UI?.Modal?.close(modalId),
            title: '关闭',
            'aria-label': '关闭',
        ...
        create('div', { class: 'modal-body' }, [
          create('div', { class: 'form-group' }, [
            create('label', { class: 'form-label' }, '摄像头名称'),
            create('input', {
              id: 'cam-f-name',
              class: 'ghost-input',
              style: 'width:100%;',
          ...
          create('div', { class: 'form-group', style: 'margin-top:12px;' }, [
            create('label', { class: 'form-label' }, '安装位置'),
            create('input', {
              id: 'cam-f-location',
              class: 'ghost-input',
              style: 'width:100%;',
              placeholder: '如：3栋1单元客厅',
              value: (camera && camera.location) || '',
            }),
          ]),
          create('div', { class: 'form-group', style: 'margin-top:12px;' }, [
            create('label', { class: 'form-label' }, '所属社区'),
            create('select', {
              id: 'cam-f-community',
              class: 'ghost-input',
              style: 'width:100%;',
              disabled: !isSys ? '' : null,
            }, isSys ? [create('option', { value: '' }, '请选择社区')] : []),
          ]),
          create('div', { class: 'btn-row', style: 'margin-top:18px;justify-content:flex-end;' }, [
            create('button', {
              class: 'btn btn-ghost',
              onclick: () => global.UI?.Modal?.close(modalId),
              text: '取消',
            ...
            create('button', {
              ...
              id: 'cam-f-submit',
              onclick: () => this._submitCameraForm(isEdit ? camera.id : null, modalId),
              text: isEdit ? '保存修改' : '创建',
            }),
          ]),
        ]),
      ...
      global.UI?.Modal?.open(modalId);

      // 加载社区选项
      if (isSys) {
        this._loadCameraCommunityOptions($('cam-f-community'), camera && camera.communityId);
      } else {
        // 小区管理员：自动填入自己的社区
        const sel = $('cam-f-community');
        if (sel && session && session.communityId) {
          sel.innerHTML = '';
          sel.appendChild(create('option', { value: session.communityId }, session.communityName || ('社区#' + session.communityId)));
          sel.value = session.communityId;
        }
      }
    },

    async _loadCameraCommunityOptions(sel, selectedId) {
      if (!sel) return;
      try {
        const page = await global.DashboardApi.Community.getList({ pageSize: 999 });
        const records = (page && page.records) || [];
        sel.innerHTML = '';
        sel.appendChild(create('option', { value: '' }, '请选择社区'));
        records.forEach(r => {
          const opt = create('option', { value: r.id }, r.name || ('社区#' + r.id));
          if (selectedId && String(r.id) === String(selectedId)) opt.selected = true;
          sel.appendChild(opt);
        });
      } catch (err) {
        console.warn('[settings] 加载社区选项失败：', err);
      }
    },

    async _submitCameraForm(id, modalId) {
      const name = $('cam-f-name') ? $('cam-f-name').value.trim() : '';
      const location = $('cam-f-location') ? $('cam-f-location').value.trim() : '';
      const communityId = $('cam-f-community') ? $('cam-f-community').value : '';

      if (!name) { try { global.UI?.Toast?.warn('请填写摄像头名称'); } catch (_) {} return; }
      if (!communityId) { try { global.UI?.Toast?.warn('请选择所属社区'); } catch (_) {} return; }

      const submitBtn = $('cam-f-submit');
      if (submitBtn) { submitBtn.disabled = true; submitBtn.textContent = '提交中…'; }

      try {
        const data = { name, location, communityId };
        if (id) {
          await global.DashboardApi.Camera.update(id, data);
          try { global.UI?.Toast?.success('摄像头已更新'); } catch (_) {}
        } else {
          await global.DashboardApi.Camera.create(data);
          try { global.UI?.Toast?.success('摄像头已创建'); } catch (_) {}
        }
        global.UI?.Modal?.close(modalId);
        this._loadCameraList();
      } catch (err) {
        try { global.UI?.Toast?.error('操作失败：' + (err && err.message || '接口待部署')); } catch (_) {}
      } finally {
        if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = id ? '保存修改' : '创建'; }
      }
    },

    async _deleteCamera(camera) {
      const ok = await global.UI?.Modal?.confirm('确认删除', '确定要删除摄像头「' + (camera.name || camera.id) + '」吗？此操作不可撤销。');
      if (!ok) return;
      try {
        await global.DashboardApi.Camera.remove(camera.id);
        try { global.UI?.Toast?.success('摄像头已删除'); } catch (_) {}
        this._loadCameraList();
      } catch (err) {
        try { global.UI?.Toast?.error('删除失败：' + (err && err.message || '权限不足或接口未部署')); } catch (_) {}
      }
    },

    async _openCameraBindForm(camera) {
      const modalId = 'camera-bind-modal';
      let modal = $(modalId);
      if (modal) modal.remove();
      ...
      modal = create('div', {
        ...
        create('div', { class: 'modal-header' }, [
          create('div', { class: 'modal-title-wrap' }, [
            ...
            create('div', {}, [
              create('div', { class: 'modal-title' }, '绑定传感器设备'),
              create('div', { class: 'modal-sub' }, '摄像头「' + (camera.name || camera.id) + '」→ 选择要绑定的传感器'),
            ]),
          ]),
          create('button', {
            class: 'modal-close',
        ...
        create('div', { class: 'modal-body' }, [
          create('div', { class: 'form-group' }, [
            create('label', { class: 'form-label' }, '选择传感器设备'),
            create('select', {
              id: 'cam-bind-device',
              class: 'ghost-input',
              style: 'width:100%;',
            }, [create('option', { value: '' }, '加载中…')]),
          ]),
          create('div', { class: 'btn-row', style: 'margin-top:18px;justify-content:flex-end;' }, [
            create('button', {
              class: 'btn btn-ghost',
            ...
            create('button', {
              class: 'btn btn-primary',
              id: 'cam-bind-submit',
              onclick: () => this._submitCameraBind(camera.id, modalId),
              text: '确认绑定',
            }),
          ]),
        ]),
      ...
      // 加载设备列表
      const sel = $('cam-bind-device');
      if (sel) {
        try {
          const devices = await global.DashboardApi.Device.getRealtimeList(999);
          sel.innerHTML = '';
          if (!devices || !devices.length) {
            sel.appendChild(create('option', { value: '' }, '暂无可绑定设备'));
          } else {
            sel.appendChild(create('option', { value: '' }, '请选择设备'));
            devices.forEach(d => {
              sel.appendChild(create('option', { value: d.id },
                '#' + d.id + ' ' + (d.name || '') + (d.location ? ' · ' + d.location : '')));
            });
          }
        } catch (err) {
          sel.innerHTML = '';
          sel.appendChild(create('option', { value: '' }, '设备列表加载失败'));
        }
      }
    },

    async _submitCameraBind(cameraId, modalId) {
      const deviceId = $('cam-bind-device') ? $('cam-bind-device').value : '';
      if (!deviceId) { try { global.UI?.Toast?.warn('请选择要绑定的设备'); } catch (_) {} return; }

      const btn = $('cam-bind-submit');
      if (btn) { btn.disabled = true; btn.textContent = '绑定中…'; }

      try {
        await global.DashboardApi.Camera.bindDevice(cameraId, deviceId);
        try { global.UI?.Toast?.success('绑定成功'); } catch (_) {}
        global.UI?.Modal?.close(modalId);
        this._loadCameraList();
      } catch (err) {
        try { global.UI?.Toast?.error('绑定失败：' + (err && err.message || '接口待部署')); } catch (_) {}
      } finally {
        if (btn) { btn.disabled = false; btn.textContent = '确认绑定'; }
      }
    },

    async _unbindCameraDevice(camera) {
      const ok = await global.UI?.Modal?.confirm('确认解绑', '确定要解除摄像头「' + (camera.name || camera.id) + '」的设备绑定吗？');
      if (!ok) return;
      try {
        await global.DashboardApi.Camera.unbindDevice(camera.id);
        try { global.UI?.Toast?.success('已解除绑定'); } catch (_) {}
        this._loadCameraList();
      } catch (err) {
        try { global.UI?.Toast?.error('解绑失败：' + (err && err.message || '接口待部署')); } catch (_) {}
      }
    },
  };
