...
(function (global) {
  ...
  const Alerts = {
    ...
    /** 设置社区过滤器（社区层视图调用），传 null 则清除过滤器（回到全局视图） */
    setCommunityFilter(communityId) {
      this._communityFilter = communityId ? String(communityId) : null;
      // 立即按新过滤器刷新告警事件流 + 统计
      this.renderStats();
      this.renderEvents();
    },
    ...
    async renderStats() {
      ...
      let data;
      try {
        // 社区层：先拉列表再前端聚合（后端无 communityId 参数时兜底）
        if (this._communityFilter) {
          data = await this._calcCommunityAlertStats(this._communityFilter);
        } else {
    ...
    /** 社区层统计：前端聚合（与 _calcCommunitySummary 思路一致） */
    async _calcCommunityAlertStats(communityId) {
      try {
        const Api = global.DashboardApi;
        const list = Api && Api.Alert && typeof Api.Alert.getEventList === 'function'
          ? await Api.Alert.getEventList(500)
          : [];
        let events = Array.isArray(list) ? list : [];
        const cid = String(communityId);
        // 按 communityId 过滤（有字段才过滤，否则退化显示全量）
        if (events.length > 0 && (events[0].communityId != null || events[0].deviceCommunityId != null)) {
          events = events.filter(a =>
            String(a.communityId || a.deviceCommunityId || '') === cid
          );
        }
        // 过滤掉已归档/误报
        events = events.filter(a => a.status !== 'archived' && a.status !== 'false_alarm');
        ...
        console.warn('[alerts] _calcCommunityAlertStats 失败', err);
    ...
    async renderEvents() {
      ...
      let list;
      try {
        list = await global.DashboardApi.Alert.getEventList(500);
      } catch (err) {
        console.warn('[alerts] events 拉取失败', err);
      }
      if (Array.isArray(list)) {
        // 1) 过滤已归档/误报（不再显示在事件流中）
        list = list.filter(a => a.status !== 'archived' && a.status !== 'false_alarm');
        // 2) 社区层：按 communityId 过滤（支持 communityId / deviceCommunityId 两个字段名）
        if (this._communityFilter) {
          const cid = String(this._communityFilter);
          const hasCid = list.length > 0 && (list[0].communityId != null || list[0].deviceCommunityId != null);
          if (hasCid) {
            list = list.filter(a => String(a.communityId || a.deviceCommunityId || '') === cid);
          }
        }
      }
      if (!list || list.length === 0) {
        host.innerHTML = `<div class="empty-placeholder" style="min-height:240px;">
          ...
          <span>${this._communityFilter ? '本社区暂无活跃告警' : '暂无活跃告警'}</span></div>`;
        return;
      }
    ...
    _alertItem(a, i) {
      ...
      // 预警事件闭环：明确标注处置状态 待确认→处置中→已消除→已归档
      const status = this._getAlertStatus(a);
      const STATUS_LABEL = {
        pending:      { text: '待确认', cls: 'warn' },
        handling:     { text: '处置中', cls: 'warn' },
        confirmed:    { text: '已确认', cls: 'danger' },
        dismissed:    { text: '已消除', cls: 'ok' },
        false_alarm:  { text: '误报',   cls: '' },
        archived:     { text: '已归档', cls: '' },
      };
      const sl = STATUS_LABEL[status] || STATUS_LABEL.pending;
      const deviceIdChip = a.deviceId
        ? `<span style="padding:1px 6px;border-radius:3px;background:rgba(37,99,235,0.08);color:#3b82f6;font-size:10px;font-weight:600;margin-right:6px;">设备 #${a.deviceId}</span>`
        : '';
      const div = create('div', {
        ...
        create('div', { class: 'alert-head' }, [
          ...
          create('span', {
            class: 'dispose-status-chip ' + sl.cls,
            style: `display:inline-block;padding:1px 8px;border-radius:10px;font-size:10px;line-height:1.6;margin-left:4px;
              ${sl.cls === 'danger' ? 'background:rgba(239,68,68,0.12);color:#f87171;border:1px solid rgba(239,68,68,0.25);' :
                sl.cls === 'ok' ? 'background:rgba(22,163,74,0.1);color:#22c55e;border:1px solid rgba(22,163,74,0.2);' :
                sl.cls === 'warn' ? 'background:rgba(251,191,36,0.1);color:#f59e0b;border:1px solid rgba(251,191,36,0.22);' :
        ...
        create('div', { class: 'alert-title', title: a.title,
          html: deviceIdChip + a.title + ' <span style="float:right;color:var(--text-dim);font-size:11px;">详情 ›</span>' }),
        create('div', { class: 'alert-desc' }, a.description || ''),
        /* —— AI 视觉复核区域（告警触发后自动拍照+识别，复核中显示loading，完成后显示图片+结果） —— */
        create('div', {
          id: 'ai-review-' + (a.id || a.deviceId || ''),
          class: 'ai-review-zone',
          style: 'display:none;margin-top:8px;padding:10px;border-radius:10px;border:1px solid rgba(168,85,247,0.2);background:rgba(168,85,247,0.04);',
        }, ''),
      ]);
      return div;
    },
    ...
    /**
     * AI 视觉复核：告警触发后自动调用
     * 流程：拉设备画面 → AI 识别 → 在告警卡片内显示复核图片与预测结果
     * @param {object} alertData — 告警对象，需含 deviceId
     */
    async triggerAiReview(alertData) {
      const deviceId = alertData && (alertData.deviceId || alertData.id);
      if (!deviceId) return;

      const zoneId = 'ai-review-' + (alertData.id || deviceId);
      const zone = document.getElementById(zoneId);
      if (!zone) return; // 卡片可能已不在DOM中

      // 防重入：同一告警只复核一次
      if (zone.dataset.reviewed === '1') return;
      zone.dataset.reviewed = '1';

      // 显示复核区域 + loading
      zone.style.display = '';
      zone.innerHTML = `
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px;">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#a855f7" stroke-width="2"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7z"/><circle cx="12" cy="12" r="3"/></svg>
          <span style="font-size:11px;font-weight:600;color:#c084fc;letter-spacing:0.5px;">AI 视觉复核</span>
          <span style="font-size:10px;color:var(--text-dim);">正在拉取传感器画面...</span>
        </div>`;
      ...
      try {
        ...
        let snapshot = null;
        try {
          snapshot = await global.DashboardApi.AiRecognize.getSnapshot(deviceId);
        } catch (snapErr) {
          zone.innerHTML = `
            <div style="display:flex;align-items:center;gap:6px;">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#a855f7" stroke-width="2"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7z"/><circle cx="12" cy="12" r="3"/></svg>
              <span style="font-size:11px;font-weight:600;color:#c084fc;">AI 视觉复核</span>
              <span style="font-size:10px;color:var(--text-dim);">传感器画面获取失败（接口待部署）</span>
            </div>`;
          return;
        }
        ...
        if (!snapshot || !snapshot.image) {
          zone.innerHTML = `
            <div style="display:flex;align-items:center;gap:6px;">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#a855f7" stroke-width="2"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7z"/><circle cx="12" cy="12" r="3"/></svg>
              <span style="font-size:11px;font-weight:600;color:#c084fc;">AI 视觉复核</span>
              <span style="font-size:10px;color:var(--text-dim);">传感器未返回画面数据</span>
            </div>`;
          return;
        }

        // 2) 显示复核图片
        const imgSrc = 'data:image/jpeg;base64,' + snapshot.image;
        const time = snapshot.time || new Date().toLocaleString('zh-CN');
        const devName = snapshot.deviceName || alertData.deviceName || ('设备#' + deviceId);
        ...
        zone.innerHTML = `
          ...
          <div style="display:flex;gap:12px;flex-wrap:wrap;">
            <img src="${imgSrc}" alt="${devName} 复核画面" style="width:160px;height:120px;object-fit:cover;border-radius:8px;border:1px solid rgba(168,85,247,0.25);flex-shrink:0;" />
            <div id="ai-review-result-${deviceId}" style="flex:1;min-width:160px;display:flex;align-items:center;">
              <div style="font-size:11px;color:var(--text-dim);">
                <span style="display:inline-block;width:14px;height:14px;border:2px solid rgba(168,85,247,0.2);border-top-color:#a855f7;border-radius:50%;animation:spin 0.8s linear infinite;vertical-align:-2px;margin-right:4px;"></span>
                AI 正在分析画面...
              </div>
            </div>
          </div>`;
        ...
        // 3) 调用 AI 识别
        const resultArea = document.getElementById('ai-review-result-' + deviceId);
        try {
          const res = await global.DashboardApi.AiRecognize.recognize(snapshot.image, null);
          if (resultArea && res && res.answer) {
            resultArea.innerHTML = `
              <div style="padding:8px 10px;border-radius:8px;background:rgba(34,211,238,0.06);border:1px solid rgba(34,211,238,0.15);">
                <div style="font-size:11px;font-weight:600;color:var(--cyan);margin-bottom:4px;">AI 预测结果</div>
                <div style="font-size:12px;color:var(--text-main);line-height:1.6;white-space:pre-wrap;word-break:break-word;">${res.answer}</div>
              </div>`;
          } else if (resultArea) {
            resultArea.innerHTML = '<div style="font-size:11px;color:var(--text-dim);">AI 未返回有效结果</div>';
          }
        } catch (recErr) {
          if (resultArea) {
            resultArea.innerHTML = `<div style="font-size:11px;color:var(--text-dim);">AI 识别接口待部署</div>`;
          }
        }
      } catch (err) {
        zone.innerHTML = `<div style="font-size:11px;color:var(--text-dim);">AI 复核失败：${err && err.message || ''}</div>`;
      }
    },
    ...
    async _onDismissEmergency(a) {
      ...
      try {
        ...
        // 操作后数据同步：刷新告警统计/事件流/设备概览，保证其他角色通过 WS 或手动刷新可见一致
        const Overview = global.OverviewComponent || global.Overview;
        await Promise.all([
          this.renderStats(), this.renderEvents(),
          Overview && typeof Overview.refresh === 'function' ? Overview.refresh().catch(() => {}) : Promise.resolve(),
          global.DashboardApi && global.DashboardApi.Sensor && global.DashboardApi.Sensor.invalidateList
            ? global.DashboardApi.Sensor.invalidateList() : Promise.resolve(),
        ]);
      ...
      UI.Drawer.open({
        ...
        infoPairs: [
          ...
          ...(merged.escalated ? [{ label: '升级状态', value: '已升级 · 火警超时未确认',
              cls: 'danger' }] : []),
          { label: '设备编号',   value: merged.deviceId || merged.device || '--', cls: '' },
    ...
    async openSummary() {
      ...
      UI.Drawer.open({
        ...
        infoPairs: [
          ...
          { label: '处置值班', value: merged.operator || '--', full: true },
