...
(function (global) {
  ...
  const Login = {
    ...
    _bindRegTabs() {
      ...
      tabs.forEach(tab => {
        tab.addEventListener('click', () => {
          ...
          // 小区字段显隐逻辑：
          // - 居民：必选"归属小区"
          // - 小区管理员：不选小区（由系统管理员审核时指定管理范围），隐藏整个字段
          // - 消防员：可选"责任小区"（不选则负责全区）
          const cField = document.getElementById('reg-community');
          const cFieldRow = cField ? cField.closest('.field') : null;
          if (cField) {
            if (this.regRole === 'community_admin') {
              // 小区管理员：隐藏小区选择字段
              if (cFieldRow) cFieldRow.style.display = 'none';
              cField.required = false;
              cField.value = '';
            } else if (this.regRole === 'firefighter') {
              if (cFieldRow) cFieldRow.style.display = '';
              cField.required = false;
    ...
    async _fillRegCommunityOptions() {
      ...
      // 仅使用后端公开的小区列表（GET /community/public）。注册数据必须来自数据库，
      // 不再把前端演示常量当成可注册的小区，避免注册后产生无效归属。
      try {
        const Api = global.DashboardApi;
        if (Api && Api.Community && typeof Api.Community.getPublicList === 'function') {
          const records = await Api.Community.getPublicList();
          (records || []).forEach(c => {
          ...
          sel.innerHTML = '<option value="">当前暂无可注册小区</option>';
          sel.disabled = true;
          return;
        }
      } catch (err) {
        console.warn('[login] 从后端加载小区列表失败：', err);
      }
    ...
    async _doRegister() {
      ...
      // 小区管理员不选小区（由系统管理员审核时指定）；消防员可不选小区（负责全区）；居民必须选
      const isCA = this.regRole === 'community_admin';
      const isFF = this.regRole === 'firefighter';
      if (!communityId && !isCA && !isFF) {
        this._showRegError(cEl.disabled ? '小区列表尚未加载，请刷新页面重试' : '请选择归属小区');
        cEl.focus();
        return false;
      }
      ...
      try {
        ...
        if (okBox) {
          const tips = {
            user: '✅ 注册成功！您的账号已提交至所属小区管理员审核，审核通过后即可登录。',
            community_admin: '✅ 注册成功！您的管理员申请已提交至系统管理员审核，审核通过后将指定您管理的小区。',
            firefighter: '✅ 注册成功！您的消防员账号已提交至系统管理员审核，审核通过后即可登录。',
          };
          okBox.innerHTML = tips[this.regRole] || tips.user;
          okBox.hidden = false;
        }
        // 清空注册表单
