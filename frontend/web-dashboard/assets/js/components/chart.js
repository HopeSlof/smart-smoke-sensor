...
(function (global) {
  ...
  const Chart = {
    ...
    async renderMetrics() {
      ...
      const isPreview = !!(trend && trend.preview);
      ...
      const items = [
        { cls: 'green',  value: avg  != null ? avg.toFixed(1)  : '12.0',  label: '平均浓度 μg/m³', bar: ratioBar(avg) },
        { cls: 'warn',   value: peak != null ? peak.toFixed(1) : '15.0',  label: '峰值 μg/m³',    bar: ratioBar(peak) },
        { cls: exceed > 0 ? 'danger' : '', value: String(exceed),         label: '超限次数',       bar: null },
        { cls: '',       value: '2',                                       label: '监测区域',       bar: null },
      ];
      render(host, items.map(it => {
        const v = (it.value === '' || it.value == null) ? '--' : it.value;
        const sub = isPreview ? create('div', { class: 'metric-sub' }, '示范预览') : null;
        const children = [
    ...
    _openExpandModal() {
      ...
      title.appendChild(create('span', { class: 'ce-badge' }, rangeLabel + (trend.preview ? ' · 示范预览' : '')));
      header.appendChild(title);
      const closeBtn = create('button', { class: 'ce-close', title: '关闭 (Esc)' });
    ...
    _bindSubHoverTooltip(svg, host, trend, m) {
      ...
      overlay.addEventListener('mousemove', (ev) => {
        ...
        const html = `<div class="chart-tooltip">
          <div class="tt-time">⏱ ${trend.xLabels[i]}${trend.preview ? ' · 示范预览' : ''}</div>
    ...
    async renderTrend() {
      ...
      // —— 无真实上报数据时，回退到「基线预览」趋势 ——
      // 围绕正常室内空气基线值（烟雾 12 / 温度 26 / CO 6）生成小幅自然波动，
      // 让趋势图可视化能力可见，同时用「示范预览数据」徽章明确告知非真实读数。
      let isBaseline = false;
      if (!trend || !trend.xLabels || trend.xLabels.length === 0) {
        trend = Chart._genBaselineTrend(this._deviceId, this._range);
        isBaseline = true;
      }
      trend.preview = isBaseline;
    ...
    /**
     * 生成基线预览趋势（无硬件接入时的参考可视化）
     * 以 deviceId 为种子做确定性伪随机，保证同一设备每次刷新波形一致。
     * 基线值：烟雾 12μg/m³ / 温度 26℃ / CO 6ppm，叠加 ±小幅正弦波动。
     */
    _genBaselineTrend(deviceId, range) {
      const hours = { '1h': 1, '6h': 6, '12h': 12, '24h': 24 }[range] || 24;
      // 采样点数：1h→12 点（5min），6h→12 点（30min），12h→12 点（1h），24h→12 点（2h）
      const N = 12;
      // 确定性种子：用 deviceId 数值化做种子
      const seed = Number(String(deviceId).replace(/\D/g, '')) || 1;
      const rnd = (i) => {
        // 简易确定性伪随机（基于种子 + 索引），输出 [0,1)
        const x = Math.sin(seed * 999 + i * 137) * 10000;
        return x - Math.floor(x);
      };
      const end = new Date();
      const xLabels = [];
      for (let i = N - 1; i >= 0; i--) {
        const t = new Date(end.getTime() - i * (hours / N) * 3600 * 1000);
        const p = n => String(n).padStart(2, '0');
        xLabels.push(`${p(t.getHours())}:${p(t.getMinutes())}`);
      }
      // 基线 + 正弦波 + 小噪声，模拟「正常室内空气」缓变趋势
      const smoke = [], temp = [], co = [];
      for (let i = 0; i < N; i++) {
        const phase = (i / N) * Math.PI * 2;
        smoke.push(+(12 + Math.sin(phase) * 2.5 + (rnd(i) - 0.5) * 1.5).toFixed(1));
        temp.push(+(26 + Math.sin(phase + 0.6) * 1.8 + (rnd(i + 100) - 0.5) * 0.8).toFixed(1));
        co.push(+(6 + Math.sin(phase + 1.2) * 1.2 + (rnd(i + 200) - 0.5) * 0.6).toFixed(1));
      }
      return {
        xLabels,
        smoke,
        ...
        peak: { index: N - 1, value: smoke[N - 1] },
      };
    },
    ...
    _buildTrendSvg(trend) {
      ...
      /* ---- 示范预览数据徽章（无真实硬件上报时显示） ---- */
      if (trend.preview) {
        const badgeG = createSVG('g', { 'data-layer': 'preview-badge' });
        badgeG.innerHTML = `
          <rect x="${VB_W - PAD_RIGHT - 116}" y="${PAD_TOP + 2}" width="112" height="18" rx="9"
                fill="rgba(0,229,255,0.1)" stroke="rgba(0,229,255,0.45)" stroke-width="1"/>
          <circle cx="${VB_W - PAD_RIGHT - 106}" cy="${PAD_TOP + 11}" r="3.5" fill="#00e5ff">
            <animate attributeName="opacity" values="1;0.3;1" dur="2s" repeatCount="indefinite"/>
          </circle>
          <text x="${VB_W - PAD_RIGHT - 98}" y="${PAD_TOP + 15}" fill="#00e5ff" font-size="10"
                font-family="Noto Sans SC, sans-serif" letter-spacing="0.5">示范预览数据</text>`;
      ...
      overlay.addEventListener('mousemove', (ev) => {
        ...
        const html = `
          <div class="chart-tooltip">
            <div class="tt-time">⏱ ${trend.xLabels[i]}  (索引 ${i + 1}/${trend.xLabels.length})${trend.preview ? '  · 示范预览' : ''}</div>
            ${rows.join('')}
          </div>
        `;
        global.UI.Tooltip.show(ev, html, '');
