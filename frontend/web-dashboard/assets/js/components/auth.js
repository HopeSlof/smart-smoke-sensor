...
(function (global) {
  ...
  const Auth = {
    ...
    setSession({ role, username, token, userId, communityId, realName, phone }) {
     ...
     * 后端强制 role=RESIDENT + status=PENDING，忽略前端传入 role；
     * 注册成功后不自动登录（需等待管理员审核通过，status=ACTIVE 后才能登录）。
     * @param {object} param { username, password, communityId, realName, phone }
