...
(function (global) {
  ...
  /** 简易确定性基线预览数据（无真实后端数据时使用） */
  function _baselinePreview(range, seed) {
    const s = Number(seed || 1);
    const hours = { '1h': 1, '6h': 6, '12h': 12, '24h': 24 }[range] || 24;
    const points = Math.max(24, hours * 6);
    const arr = [];
    const now = new Date();
    ...
    for (let i = 0; i < points; i++) {
      ...
      // 以伪随机相位叠加正弦波动
      const k = i + s;
      const smoke = 12 + 4 * Math.sin(k / 3) + ((k * 13) % 7) * 0.3;
      const temp  = 26 + 3 * Math.sin(k / 5 + 1) + ((k * 7) % 5) * 0.2;
      const co    = 6  + 2 * Math.sin(k / 4 + 2) + ((k * 11) % 4) * 0.25;
      const hm = ('0' + t.getHours()).slice(-2) + ':' + ('0' + t.getMinutes()).slice(-2);
      arr.push({ time: hm, smoke, temperature: temp, co });
    }
    return arr;
  }
  ...
  const TrendModal = {
    ...
    async open({ deviceId, deviceName, location, model }) {
      ...
      const name = deviceName || ('设备 ' + String(deviceId).slice(-5));
      const UI = global.UI;
      ...
      try {
        ...
        const data = trendRaw.preview ? this._mapPreview(trendRaw) : trendRaw;
        this._renderViewport(mask, {
          deviceId, deviceName: name, location, model,
          reading, data, preview: !!trendRaw.preview, range: '24h',
        });
      } catch (err) {
    ...
    _injectStyles() {
      ...
      s.textContent = `
        ...
        .tm-preview-badge{display:inline-block;padding:2px 8px;border-radius:999px;font-size:10px;background:rgba(251,191,36,.15);color:#fbbf24;border:1px solid rgba(251,191,36,.35);margin-left:8px;animation:blink 1.3s infinite;}
    ...
    async _fetchTrend(deviceId, range) {
      ...
      // 降级：基线预览
      const arr = _baselinePreview(range, Number(deviceId) || 1);
      return {
        preview: true,
        xLabels: arr.map(p => p.time),
        smoke: arr.map(p => p.smoke),
        co: arr.map(p => p.co),
        temperature: arr.map(p => p.temperature),
      };
    },

    _mapPreview(raw) { return raw; },

    _renderViewport(mask, ctx) {
      const { deviceId, deviceName, location, model, reading, data, preview, range } = ctx;
      const UI = global.UI;
      const viewport = mask.querySelector('#tm-viewport');
      if (!viewport) return;
      ...
      grid.innerHTML = `
        ...
        <div class="tm-info-item"><div class="tm-info-label">型号</div><div class="tm-info-value">${model || r.deviceModel || 'SD-200X'}</div></div>
      ...
      metricRow.innerHTML = cfg.map(c => {
        ...
        const exceed = typeof d.exceed === 'number' ? d.exceed : 0;
        const pct = c.threshold ? Math.min(100, (num(d.max,0) / c.threshold) * 100) : 0;
        return `
          <div class="tm-metric-card">
            <div class="tm-metric-title"><span class="dot" style="background:${c.color};"></span>${c.name}${preview ? '<span class="tm-preview-badge" style="margin-left:auto;">示范预览</span>' : ''}</div>
            <div class="tm-metric-vals">
              ...
              <div class="k">超限</div><div class="v" style="${exceed ? 'color:#f87171;' : ''}">${exceed} 次</div>
            </div>
      ...
      tabs.addEventListener('click', async (e) => {
        ...
        const newData = newRaw.preview ? newRaw : newRaw;
        ...
        this._drawChart(chartHost, newData, newRaw.preview || false);
      });
      ...
      rightToolbar.innerHTML = `时间点：<b style="color:var(--text-main);">${((data && data.xLabels) || []).length}</b>${preview ? '<span class="tm-preview-badge">示范预览数据</span>' : ''}`;
      ...
      this._drawChart(chartWrap, data, !!preview);
    },

    _aggStats(data) {
      const calc = (arr, threshold) => {
        if (!Array.isArray(arr) || !arr.length) return { avg: null, max: null, min: null, exceed: 0 };
    ...
    _drawChart(host, data, preview) {
      ...
      overlay.addEventListener('mousemove', (e) => {
        ...
        tooltip.innerHTML = `
          <div style="margin-bottom:4px;"><b>${xLabels[i] || ''}</b>${preview ? ' <span style="color:#fbbf24;font-size:10px;">（示范预览）</span>' : ''}</div>
