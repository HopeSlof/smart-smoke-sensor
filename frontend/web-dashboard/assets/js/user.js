...
(function (global) {
  ...
  const UserPage = {
    ...
    _deviceCard(d) {
      ...
      const conc = typeof d.concentration === 'number' ? d.concentration.toFixed(1) : '--';
      const temp = typeof d.temp === 'number' ? d.temp.toFixed(0) : '--';
      const bat  = typeof d.battery === 'number' ? d.battery : (typeof d.battery === 'string' ? d.battery : '--');
      const clsVal = (v, thrGood, thrBad) => {
        if (v === '--') return '';
        const n = Number(v);
        if (Number.isNaN(n)) return '';
        if (n >= thrBad) return 'danger';
        if (n >= thrGood) return 'warn';
        return 'ok';
      };
      const card = create('div', {
        ...
        create('div', { class: 'device-card-head' }, [
          ...
          create('div', { class: 'device-card-title' }, [
            create('div', { class: 'device-card-name' }, d.name || '未命名设备'),
        ...
        create('div', { class: 'device-card-body' }, [
          this._mk('烟雾浓度',  conc + ' μg/m³', clsVal(conc, 50, 90)),
          this._mk('温度',      temp + ' ℃',     clsVal(temp, 45, 60)),
          this._mk('电量',      (bat === '--' ? '--' : bat + '%'),
                        typeof bat === 'number' ? (bat <= 20 ? 'warn' : 'ok') : ''),
          this._mk('信号',      d.rssi ? (d.rssi < -85 ? '较弱' : d.rssi < -70 ? '一般' : '良好') : '良好',
                        d.rssi && d.rssi < -85 ? 'warn' : 'ok'),
        ]),
        create('div', { class: 'device-card-footer' }, [
          create('span', {}, '心跳：' + (d.lastHeartbeat || '15 秒前')),
    ...
    _openDeviceDetail(d) {
      ...
      const html = `
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
          <h3 style="margin:0;font-size:16px;letter-spacing:2px;">${d.name || '设备详情'}
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px 14px;font-size:13px;">
          <div><div style="color:var(--text-dim);font-size:11px;">设备 ID</div><div class="highlight">${d.id || '--'}</div></div>
          <div><div style="color:var(--text-dim);font-size:11px;">安装位置</div><div>${d.location || '--'}</div></div>
          <div><div style="color:var(--text-dim);font-size:11px;">烟雾浓度</div><div style="font-family:var(--font-num);color:var(--cyan);">${typeof d.concentration === 'number' ? d.concentration.toFixed(1) + ' μg/m³' : '--'}</div></div>
          <div><div style="color:var(--text-dim);font-size:11px;">温度</div><div style="font-family:var(--font-num);">${typeof d.temp === 'number' ? d.temp.toFixed(1) + ' ℃' : '--'}</div></div>
          <div><div style="color:var(--text-dim);font-size:11px;">电量</div><div>${typeof d.battery === 'number' ? d.battery + '%' : '--'}</div></div>
          <div><div style="color:var(--text-dim);font-size:11px;">信号</div><div class="${(d.rssi && d.rssi < -80) ? 'warn' : 'ok'}">${d.rssi ? d.rssi + ' dBm' : '--'}</div></div>
          <div style="grid-column:span 2;"><div style="color:var(--text-dim);font-size:11px;">最近心跳</div><div>${d.lastHeartbeat || '--'}</div></div>
        </div>
        <div style="margin-top:14px;padding:10px 12px;border-radius:8px;background:rgba(239,68,68,0.06);border:1px solid rgba(239,68,68,0.18);font-size:12px;line-height:1.7;color:var(--text-muted);">
          <b style="color:#fca5a5;">温馨提示：</b>若设备长时间离线或反复告警，请通过左侧「联系管理员」报修，避免影响家庭安全。
        </div>`;
    ...
    _applyFiltersAndRender() {
      ...
      render(host, list.map(a => create('div', {
        class: 'alert-row',
        title: a.description || a.title,
      }, [
        create('div', { class: 'time' }, a.time || '--'),
        create('span', { class: 'level-chip ' + a.level }, LEVEL_TEXT[a.level] || '--'),
        create('div', { class: 'desc' }, [
          create('b', {}, (a.title || '--')),
          a.description ? create('span', { style: 'margin-left:6px;color:var(--text-dim);' }, '· ' + a.description) : null,
        ]),
